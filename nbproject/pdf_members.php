...
$fill=0;
	...
	while ($de = $result->fetch_assoc()) {
        ...
        $this->Cell(195, 6, "MEMBERS LIST", 0, 0, "C", 0);
...
$pdf->Cell(60, 7, "Member Name", 1, 0, "C", 1);
$pdf->Cell(40, 7, "Phone", 1, 0, "C", 1);
$pdf->Cell(70, 7, "", 1, 0, "C", 1);
...
$statement = "SELECT * FROM `member` ORDER BY `first_name` ASC, `middle_name` ASC";
include('dao/connect.php');
...
$num++;
	...
	$pdf->Cell(60, 7, str_replace("&","'",$row['first_name'])." ".str_replace("&","'",$row['middle_name'])." ".str_replace("&","'",$row['last_name']), 1, 0, "L", $fill);
	$pdf->Cell(40, 7, $row['phone'],  1, 0, "L", $fill);
