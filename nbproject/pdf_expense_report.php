...
//Page header
    function Header() {
        ...
        $this->Ln();
        if(isset($_GET['id']) ){
        $this->Cell(195, 6, "EXPENSES REPORT FOR ". getLand($_GET['id']), 0, 0, "C", 0);
        }else{
            $this->Cell(195, 6, "EXPENSES REPORT FOR ALL LANDS", 0, 0, "C", 0);
        }
...
$pdf->Cell(35, 7, "Reference", 1, 0, "C", 1);
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(35, 7, str_replace("&", "'", $row['expense_type']), 1, 0, "L", $fill);
    $pdf->Cell(35, 7, str_replace("&", "'", $row['name']), 1, 0, "L", $fill);
    $pdf->Cell(23, 7, date_format(date_create($row['expense_date']), "d-m-Y"), 1, 0, "L", $fill);
    
    $pdf->Cell(30, 7, $payment_mode, 1, 0, "L", $fill);
    $pdf->Cell(35, 7, $row['details'], 1, 0, "L", $fill);
...
$pdf->Cell(25, 7, "Amount", 1, 0, "C", 1);
    ...
    $y += 7;
    $fill = !$fill;
    $pdf->Cell(133, 7, "Total Expenses", 1, 0, "C", $fill);
