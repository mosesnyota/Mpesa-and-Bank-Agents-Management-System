...
$pdf->setXY($x, $y);
 ...
 $statement = "SELECT `company_name`,`contact_person`, invoice.* FROM invoice JOIN `suppliers` ON invoice.`supplier_id` = suppliers.`supplier_id` where pay_status='Unpaid' ORDER BY `invoice_date` DESC ";
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(50, 7, $row['company_name'], 1, 0, "L", $fill);
    $pdf->Cell(50, 7, $row['contact_person'], 1, 0, "L", $fill);
    $pdf->Cell(30, 7, $row['invoice_date'], 1, 0, "R", $fill);
    $pdf->Cell(30, 7, $row['amount'], 1, 0, "R", $fill);
    $pdf->Cell(20, 7, $row['pay_status'], 1, 0, "R", $fill);
